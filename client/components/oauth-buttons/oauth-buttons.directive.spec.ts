...
describe('Directive: oauthButtons', function() {
  ...
  it('should contain anchor buttons', function() {
    ...
    expect($(element[0]).find('a.btn').length).to.be.at.least(1);
  });
  ...
  it('should bind scope.classes to class names on the anchor buttons', function() {
    ...
    elementScope.$digest();
    expect($(element[0]).find('a.btn.testClass1.testClass2').length).to.be.at.least(1);

    // Remove classes
    elementScope.classes = '';
    elementScope.$digest();
    expect($(element[0]).find('a.btn.testClass1.testClass2').length).to.equal(0);
  });
});
